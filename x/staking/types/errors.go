...
var (
	...
	ErrNotEnoughBalance                         = errorsmod.Register(ModuleName, 42, "not enough balance")
	ErrTokenizeShareRecordNotExists             = errorsmod.Register(ModuleName, 43, "tokenize share record not exists")
	ErrTokenizeShareRecordAlreadyExists         = errorsmod.Register(ModuleName, 44, "tokenize share record already exists")
	ErrNotTokenizeShareRecordOwner              = errorsmod.Register(ModuleName, 45, "not tokenize share record owner")
	ErrExceedingFreeVestingDelegations          = errorsmod.Register(ModuleName, 46, "trying to exceed vested free delegation for vesting account")
	ErrOnlyBondDenomAllowdForTokenize           = errorsmod.Register(ModuleName, 47, "only bond denom is allowed for tokenize")
	ErrInsufficientValidatorBondShares          = errorsmod.Register(ModuleName, 48, "insufficient validator bond shares")
	ErrRedelegationNotAllowedForValidatorBond   = errorsmod.Register(ModuleName, 49, "redelegation is not allowed for validator bond delegation")
	ErrValidatorBondNotAllowedForTokenizeShare  = errorsmod.Register(ModuleName, 50, "validator bond delegation is not allowed to tokenize share")
	ErrValidatorBondNotAllowedFromModuleAccount = errorsmod.Register(ModuleName, 51, "validator bond is not allowed from a module account")
	ErrGlobalLiquidStakingCapExceeded           = errorsmod.Register(ModuleName, 52, "delegation from liquid staking provider exceeds the global cap")
)
