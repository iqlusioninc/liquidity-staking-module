...
Currently LSM facilites the immediate conversion of staked assets into liquid staked tokens (process hereinafter referred to as "tokenization"). Despite the many benefits that come with this capability, it does inadvertently negate a protective measure available with traditional staking, where a user can stake their tokens, rendering them inaccessible in the event that their wallet is compromised. 

As an additional protective measure, this proposal incorporates a feature to permit users to selectively disable the tokenization of their stake.


## Proposal
This proposal grants user's the ability to disable and reactivate the tokenization of their stake. When tokenization is disabled, a lock is placed on the user's account, effectively preventing the conversion of any of their delegations. Re-enabling tokenization would initiate the removal of the lock, but the process is not immediate. The lock removal is queued, with the lock itself persisting throughout the unbonding period. Following the completion of the unbonding period, the lock would be completely removed, restoring the user's ablility to tokenize. This delay better positions users to regain control of their funds.
...
### Queries
```proto
// QueryExpiringTokenizeShareLock queries whether the completion
// time for an expiring tokenize share lock
message QueryExpiringTokenizeShareLock {
  string address = 1;
}
message QueryExpiringTokenizeShareLockResponse{
  string completion_time = 1;
}
```
