...
ADR-002 introduces the global liquid staking cap, validator liquid staking cap, and renames exemption factor to validator bond factor. Delegations from module account and LSM tokenized shares are tracked against the global liquid staking, validator liquid staking cap, and validator bond caps. This requires changing the standard staking transactions to introspect. The reason for tracking module account shares is because ICAs are module accounts under the hood, so in practice this limits liquid staking.
...
New governance parameters are introduced that define the cap on the percentage of delegated shares than can be liquid, namely the `GlobalLiquidStakingCap` and `ValidatorLiquidStakingCap`. Additionally, the `ExemptionFactor` has been renamed to `ValidatorBondFactor`, and a boolean parameter `LiquidStakedCapsEnabled` has been added to allow the toggling of the these additional safety thresholds.
...
message Params {
  ...
  bool liquid_staking_caps_enabled = 10;
}
```
