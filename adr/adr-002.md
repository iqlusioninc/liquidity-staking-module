...
Introduces the global liquid staking cap and renames the exemption factor to validator bond factor. Both module account and LSM tokenized shares are tracked against the validator bond and global liquid staking cap. This requires changing the standard staking transactions to introspect. The reason for tracking module account shares against validator bond is because ICAs are module accounts under the hood, so in practice this limits liquid staking with validator bonds. 
...
func BeginRedelegate() {
    ...
    //    Decrement destination validator liquid shares
}
...
With the global liquid staking cap and validator bond cap in consideration, there are additional scenarios that would cause a transaction to fail:
...
After upgrading to enable the liquid staking module, the total amount liquid staked and total liquid validator shares must be determined. This can be done in the upgrade handler by looping through delegation records and including the delegation in the total if the delegator is a module account.